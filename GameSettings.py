...
class GameSettings():
    ...
    def canAddPlayer(self): #Returns true if a player can be added
        return (self.numPlayers < 4)
    
    def canRemovePlayer(self):  #Returns true if a player can be removed
        return (self.numPlayers > 0)
    
    def canBegin(self): #Returns true if the number of players is greater than 1, which allows the game to begin
        return (self.numPlayers > 1)
        
    def addPlayer(self, player):    #Adds a player to the player staging list, which stores the players before the game starts
        self.playerStaging.append(player)
        self.numPlayers += 1
        
    def removePlayer(self, number): #Removes a pla
        number -= 1
        del self.playerStaging[number]
        self.numPlayers -= 1
        
    def clearStaging(self): #Clears the number of players and player objs from the staging list
        self.numPlayers = 0
        self.playerStaging = []
